...
makeSuite('PoolConfigurator', (testEnv: TestEnv) => {
  ...
  it('Activates the PriceOracleSentinel for ETH reserve by pool Admin', async () => {
    const { configurator, weth, helpersContract } = testEnv;
    expect(await configurator.setPriceOracleSentinelActive(weth.address, true))
  ...
  it('Deactivates the PriceOracleSentinel for ETH reserve by Pool admin', async () => {
    const { configurator, helpersContract, weth } = testEnv;
    expect(await configurator.setPriceOracleSentinelActive(weth.address, false))
  ...
  it('Activates the PriceOracleSentinel for ETH reserve by Risk Admin', async () => {
    const { configurator, weth, helpersContract, riskAdmin } = testEnv;
    expect(
      await configurator.connect(riskAdmin.signer).setPriceOracleSentinelActive(weth.address, true)
    )
      .to.emit(configurator, 'PriceOracleSentinelActivated')
      .withArgs(weth.address);
  ...
  it('Deactivates the PriceOracleSentinel for ETH reserve by Risk admin', async () => {
    const { configurator, helpersContract, weth, riskAdmin } = testEnv;
    expect(
      await configurator.connect(riskAdmin.signer).setPriceOracleSentinelActive(weth.address, false)
    )
      .to.emit(configurator, 'PriceOracleSentinelDeactivated')
      .withArgs(weth.address);
